...
class SearchController
{
	...
	public function actionSearchParams()
	{
		...
		if ($_POST['age1'] && $_POST['age2']) {
			...
			$stmt = $pdo->prepare("SELECT id, login, avatar FROM user WHERE id IN (SELECT id_user FROM user_info WHERE birthday BETWEEN :age1 AND :age2)");
			$stmt->bindParam(':age1', $params['age2'], PDO::PARAM_STR);
			$stmt->bindParam(':age2', $params['age1'], PDO::PARAM_STR);
			$stmt->execute();
